...
import { renderValidationError } from "../components/ui/messages/validationError.js";
...
export function showFieldValidationErrors(form, errors) {
  ...
  errors.forEach((err) => {
    ...
    // Select input by name attribute (safe selector)
    const input = form.querySelector(`[name="${CSS.escape(err.field)}"]`);
    if (!input) return;

    // Find nearest form-group wrapper (safe error placement)
    const fieldWrapper =
